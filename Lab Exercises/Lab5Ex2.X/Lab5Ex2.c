...
{ 
  ...
  ANSELH = 0x00; 
    ...
    while(1){                   // Infinite do?nothing loop
        ...
        char lastName []={"Wood"};//creates a string
        LCD_initialise();//initialises the LCD
        LCD_puts(firstName);//displays firstName on the LCD
        LCD_cursor(0,1);//moves the cursor to the bottom line
        LCD_puts(lastName);//displays lastName on the LCD
        __delay_ms(500);
...
{ 
    ...
    static unsigned short count = 0;//initialises the count variable
