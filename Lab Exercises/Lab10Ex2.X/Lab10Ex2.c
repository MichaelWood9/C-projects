 ...
 { 
   ...
   while(1) 
    { 
       ...
       if (count_Down == 0){//checks count down is 0
          CCPR2L++;           // Change PWM duty cycle 
       }
       if (count_Down == 1){//check if countdown is 1
           CCPR2L--;//decreases the PWM duty cycle
       }
        
       if(CCPR2L == 100){
           count_Down = 1;//sets count down to 1 to start counting back down
       }
    }       
} 
