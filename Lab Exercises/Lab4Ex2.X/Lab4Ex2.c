...
#define _XTAL_FREQ 4000000  // MCU clock speed ? required for delay macros 
 void main(void) 
{ 
    unsigned char preload = 131; // TMR0 preload 2ms/overflow@ 4Mhz,1:16 prescaler 
  ...
  ANSELH = 0x00; 
    ...
    INTCONbits.T0IF = 0;          // Reset Timer0 interrupt flag (known state)      
    while(1)                    // Infinite loop 
    {                        
        for(i=0;i<500;i++)       // Loop to cascade several short delays together         
        { 
            TMR0 = preload;             // Preload timer0 
            while(!INTCONbits.T0IF);    // Delay loop: Wait until TMR0 overflows 
            INTCONbits.T0IF = 0;        // Reset overflow flag, TMR0IF         
        } 
